			...
			if (fixLinearization) // 固定线性化（优化后执行）
			{
				...
				if (r->efResidual->isActive()) // 残差是in的
				{
					if (r->isNew) {
						...
						float relBS   = 0.01 * ((ptp_inf.head<2>() / ptp_inf[2]) - (ptp.head<2>() / ptp[2]))
								.norm();    // 0.01 = one pixel.
...
//@ 对残差进行线性化, 并去掉不在图像内, 并且残差大的
	Vec3 FullSystem::linearizeAll(bool fixLinearization) {
		...
		if (multiThreading) {
			...
			treadReduce.reduce(boost::bind(&FullSystem::linearizeAll_Reductor, this, fixLinearization, toRemove, _1, _2,
			                               _3, _4), 0, activeResiduals.size(), 0);
...
//[ ***step 1*** ] 找出未线性化(边缘化)的残差, 加入activeResiduals
		...
		{
			auto perf = dso::PerfMonitor("Perf: FullSystemOpt | add to activeResiduals");
			for (FrameHessian* fh : frameHessians)
				for (PointHessian* ph : fh->pointHessians) {
...
//[ ***step 2*** ] 线性化activeResiduals的残差, 计算边缘化的能量值 (然而这里都设成0了)
		...
		Vec3   lastEnergy;
		double lastEnergyL, lastEnergyM;

		{
			auto perf = dso::PerfMonitor("Perf: FullSystemOpt | linearizeAll");
...
//[ ***step 3*** ] 迭代求解
		{
			auto perf = dso::PerfMonitor("Perf: FullSystemOpt | iteration");
			...
			for (int iteration = 0; iteration < mnumOptIts; iteration++) {
				...
				//[ ***step 3.2*** ] 求解系统
				{
//					auto perf_solve = dso::PerfMonitor("Perf: solveSystem");
...
//[ ***step 4*** ] 判断是否接受这次计算
		...
		{
//			auto perf = dso::PerfMonitor("Perf: FullSystemOpt | set linearization pt");
...
//@ 得到各个状态的零空间
	std::vector<VecX> FullSystem::getNullspaces(
		...
		for (int i = 0; i < 2; i++) {
			...
			nullspace_x0.setZero();
			for (FrameHessian* fh : frameHessians) {
				nullspace_x0.segment<2>(CPARS + fh->idx * 8 + 6) = fh->nullspaces_affine.col(i)
				                                                     .head<2>();  //? 这个head<2>是为什么

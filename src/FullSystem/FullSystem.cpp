...
//[ ***step 1*** ] 设置不同的运动状态
		...
		if (allFrameHistory.size() == 2)
			for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++)
				lastF_2_fh_tries.emplace_back(SE3());  //? 这个size()不应该是0么
		else {
			...
			lastF_2_fh_tries
					.push_back(SE3::exp(fh_2_slast.log() * 0.5).inverse() * lastF_2_slast); // assume half motion.
...
//[ ***step 3*** ] 如果跟踪正常, 并且0层残差比最好的还好留下位姿, 保存最好的每一层的能量值
			...
			if (haveOneGood) {
				for (int i = 0; i < 5; i++) {
					if (!std::isfinite((float) achievedRes[i]) || achievedRes[i] > coarseTracker
							->lastResiduals[i])    // take over if achievedRes is either bigger or NAN.
...
//[ ***step 3*** ] 优化上一步挑出来的未成熟点, 进行逆深度优化, 并得到pointhessian
		...
		if (multiThreading)
			treadReduce
					.reduce(boost::bind(&FullSystem::activatePointsMT_Reductor, this, &optimized, &toOptimize, _1, _2,
					                    _3, _4), 0, toOptimize.size(), 50);
...
//[ ***step 5*** ] 如果已经初始化过了，对新来的帧进行跟踪, 得到位姿光度, 判断跟踪状态
			...
			if (coarseTracker_forNewKF->refFrameID
			    > coarseTracker->refFrameID)  //[cc] coarseTracker_forNewKF as ref, coarseTracker as current
			{
...
//[ ***step 5*** ] 构建之前关键帧与当前帧fh的残差(旧的)
		...
		for (FrameHessian* fh1 : frameHessians)        // go through all active frames
		{
			...
			for (PointHessian* ph : fh1->pointHessians)  // 全都构造之后再删除
			{
				...
				r->setState(ResState::IN);  //[cc] Initialize as in status
...
//[ ***step 9*** ] 标记删除和边缘化的点, 并删除&边缘化
			...
			{
				auto perf_marg_pt = dso::PerfMonitor("Perf: makeKeyFrame | marginalizePointsF");
...
//[ ***step 10*** ] 生成新的点
			...
			{
				auto perf_make_new = dso::PerfMonitor("Perf: makeKeyFrame | makeNewTraces");
				makeNewTraces(fh, 0);
			}

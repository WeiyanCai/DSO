	...
	void EnergyFunctional::setAdjointsF(CalibHessian* Hcalib) {
		...
		for (int h = 0; h < nFrames; h++) // 主帧
			for (int t = 0; t < nFrames; t++) // 目标帧
			{
				...
				AH.topLeftCorner<6, 6>() = -hostToTarget.Adj()
...
//@ 计算零空间矩阵伪逆, 从 H 和 b 中减去零空间, 相当于设相应的Jacob为0
	void EnergyFunctional::orthogonalize(VecX* b, MatXX* H) {
		...
		MatXX NNpiTS = 0.5 * (NNpiT
		                      + NNpiT.transpose());    // = N * (N' * N)^-1 * N'. //[cc]calculate (NN`)T as 0.5*(NN` + (NN`)T)
...
//[ ***step 3*** ] 使用SVD求解, 或者ldlt直接求解
		...
		if (setting_solverMode & SOLVER_SVD) {
			...
			//! Hx=b --->  U∑V^T*x = b
			{
				auto perf = dso::PerfMonitor("Perf: Eigen Jacobi SVD");
			...
			VecX  SVecI        = (HFinal_top.diagonal() + VecX::Constant(HFinal_top.cols(), 10)).cwiseSqrt()
			                                                                                    .cwiseInverse();
			MatXX HFinalScaled = SVecI.asDiagonal() * HFinal_top * SVecI.asDiagonal();
			x = SVecI.asDiagonal()

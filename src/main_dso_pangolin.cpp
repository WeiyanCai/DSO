...
int main(int argc, char** argv) {
	...
	// Reader 负责解析数据，source 就是命令行填的 files
	...
	std::thread runthread([&]() {
		...
		for (int            i = lstart; i >= 0 && i < reader->getNumImages() && linc * i < linc * lend; i += linc) {
			...
			if (timesToPlayAt.size() == 0) {
				...
				timesToPlayAt.push_back(timesToPlayAt.back() + fabs(tsThis - tsPrev)
				                                               / playbackSpeed); //[cc] ccording playbackSpeed set new ts
			}
		}
		...
		printf("\n======================"
		       ...
		       "\n%.3fx (multi core); "
		       "\n======================\n\n",
		       numFramesProcessed, numFramesProcessed / numSecondsProcessed,  //[cc] desired FPS according to image ts
		       MilliSecondsTakenSingle / numFramesProcessed,                  //[cc] actual spent time
